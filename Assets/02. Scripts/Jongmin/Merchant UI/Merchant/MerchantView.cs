...
public class MerchantView : MonoBehaviour, IMerchantView
{
    [Header("UI 관련 컴포넌트")]
    [Header("캔버스 그룹")]
    ...
    [Space(30f), Header("테스트 기능")]
    [Header("열기 버튼")]
    [SerializeField] private Button m_open_button;

    private Coroutine m_fade_coroutine;
    private MerchantPresenter m_presenter;

    public void Inject(MerchantPresenter presenter)
    {
    ...
    private IEnumerator ToggleActive(bool is_in)
    {
        ...
        if(!is_in)
            m_presenter.ToggleDialogue(is_in);
        ...
        while(elapsed_time < target_time)
        {
            ...
            var target_alpha = is_in ? delta : 1f - delta;
